...
export function MatchDetails() {
  ...
  const handleAddMatch = async (e) => {
    ...
    if (res) {
      navigate(`/Sports/${sport}/scoring?m-id=${res}`);
      // navigate(-1);
    }
  }
                ...
                <div className="flex items-center justify-between border border-input px-3 py-2  shadow-xs rounded-md col-span-2">
                      ...
                      <Switch
                          ...
                          onCheckedChange={(checked) => 
                              {
                                setIsTeam(checked)
                                setFormData({ ...formData, is_team: checked })
                                }}
                            />
                          </div>
                        <div className="flex flex-col gap-2 ">
                         ...
                         <Popover open={openA} onOpenChange={setOpenA}>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  Select {is_team ? "Team A" : "Player A"}
                              ...
                              <PopoverContent className="w-full p-0">
                                <Command>
                                  ...
                                  <CommandList>
                                    ...
                                    <CommandGroup>
                                        {is_team ? (filterA?.map((t) => (
                                          <CommandItem
                                            key={t.team_id}
                                            value={t.team_id}
                                            onSelect={() => togglePlayerA(t.team_id)}
                                          >
                                            ...
                                            <Check
                                              className={cn(
                                                "ml-auto h-4 w-4",
                                                selectedA === t.team_id ? "opacity-100" : "opacity-0"
                                              )}
                                            />
                                          </CommandItem>
                                        ))) : (filterA?.map((p) => (
                                          <CommandItem
                                            key={p.player_id}
                                            value={p.player_id}
                                            onSelect={() => togglePlayerA(p.player_id)}
                                          >
                                            {capitalizeFirstLetter(p.first_name)} &nbsp;
                                            {capitalizeFirstLetter(p.last_name)}

                                            <Check
                                              className={cn(
                                                "ml-auto h-4 w-4",
                                                selectedA === p.player_id ? "opacity-100" : "opacity-0"
                                              )}
                                            />
                                          </CommandItem>
                                        )))}
                         ...
                         <Popover open={openB} onOpenChange={setOpenB} >
                              <PopoverTrigger asChild>
                                <Button
                                  ...
                                  className="w-full justify-between"
                                >
                                  Select {is_team ? "Team B" : "Player B"}
                                  <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
                                </Button>
                              </PopoverTrigger>
                              <PopoverContent className="w-full p-0">
                                <Command>
                                  ...
                                  <CommandList>
                                    <CommandEmpty>No {is_team ? "teams" : "players"} found.</CommandEmpty>
                                    <CommandGroup>
                                        {is_team ? (filterB?.map((t) => (
                                          <CommandItem
                                            key={t.team_id}
                                            value={t.team_id}
                                            onSelect={() => togglePlayerB(t.team_id)}
                                          >
                                            {capitalizeFirstLetter(t.name)} 
                                            ...
                                            <Check
                                              className={cn(
                                                "ml-auto h-4 w-4",
                                                selectedB === t.team_id ? "opacity-100" : "opacity-0"
                                              )}
                                            />
                                          </CommandItem>
                                        ))) : (filterB?.map((p) => (
                                          <CommandItem
                                            key={p.player_id}
                                            value={p.player_id}
                                            onSelect={() => togglePlayerB(p.player_id)}
                                          >
                                            {capitalizeFirstLetter(p.first_name)} &nbsp;
                                            {capitalizeFirstLetter(p.last_name)}
                                            ...
                                            <Check
                                              className={cn(
                                                "ml-auto h-4 w-4",
                                                selectedB === p.player_id ? "opacity-100" : "opacity-0"
                                              )}
                                            />
                                          </CommandItem>
                                        )))}
