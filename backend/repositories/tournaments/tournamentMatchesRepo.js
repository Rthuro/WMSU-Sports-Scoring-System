...
export async function create(data) {
  const result = await sql`
    ...
    VALUES (${data.match_id}, ${data.sport_id}, ${data.tournament_id}, ${data.match_name}, ${data.date}, ${data.start_time}, ${data.end_time}, ${data.location}, ${data.team_a_id}, ${data.team_b_id}, ${data.round})
    RETURNING *
  `;
  return result[0];
}
...
export async function update(id, data) {
    const teamA = data.team_a_id === 'empty' ? null : data.team_a_id;
    const teamB = data.team_b_id === 'empty' ? null : data.team_b_id;
    ...
    const result = await sql`
        ...
        SET match_name = COALESCE(${data.match_name}, match_name),
            ...
            team_a_id = CASE WHEN ${data.team_a_id !== undefined} THEN ${teamA} ELSE team_a_id END,
            team_b_id = CASE WHEN ${data.team_b_id !== undefined} THEN ${teamB} ELSE team_b_id END,
